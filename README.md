...
My table has an autogenerated id Field. description field and Completed fields.  
The table has a many to one relationship with the users table. 
2. Can you explain how you connected your API to a database?
imported a sql database in the pom file and configured in the application properties
then in the repository, we extend to the CrudRepsitory
3. Can you explain the flow of data from client through the backend system and back to the client?
the client will make an api call to the back end using  a specific call and corresponding endpoint. The backend will receive the request and if its valid , it will return the requested data back to the client. if not, it will return an error. 
